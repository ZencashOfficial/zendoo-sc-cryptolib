...
use algebra::{fields::bn_382::Fr as BN382Fr, curves::bn_382::G1Projective as BN382G1Projective, Field, PrimeField, ToBits};
use primitives::{
...
use r1cs_crypto::{
    ...
    crh::{BN382FrPoseidonHashGadget, FieldBasedHashGadget},
};

use r1cs_std::{instantiated::bn_382::G1Gadget as BN382G1Gadget, fields::{
...
//Sig types
type SchnorrSigGadget = FieldBasedSchnorrSigGadget<BN382Fr, BN382G1Projective>;
type SchnorrVrfySigGadget = FieldBasedSchnorrSigVerificationGadget<
    BN382Fr, BN382G1Projective, BN382G1Gadget, BN382FrPoseidonHash, BN382FrPoseidonHashGadget
>;
type SchnorrPk = FieldBasedSchnorrPk<BN382G1Projective>;
type SchnorrPkGadget = FieldBasedSchnorrPkGadget<BN382Fr, BN382G1Projective, BN382G1Gadget>;

//Field types
type BN382FrGadget = FpGadget<BN382Fr>;

pub struct NaiveTresholdSignature<F: PrimeField>{

    //Witnesses
    pks:                      Vec<Option<SchnorrPk>>, //pk_n = g^sk_n
    sigs:                     Vec<Option<FieldBasedSchnorrSignature<BN382Fr, BN382G1Projective>>>, //sig_n = sign(sk_n, H(MR(BT), BH(Bi-1), BH(Bi)))
    threshold:                Option<BN382Fr>,
    b:                        Vec<Option<bool>>,
    end_epoch_mc_b_hash:      Option<BN382Fr>,
    prev_end_epoch_mc_b_hash: Option<BN382Fr>,
    mr_bt:                    Option<BN382Fr>,
...
impl<F: PrimeField>NaiveTresholdSignature<F> {
    pub fn new(
        pks:                      Vec<SchnorrPk>,
        sigs:                     Vec<Option<FieldBasedSchnorrSignature<BN382Fr, BN382G1Projective>>>,
        threshold:                BN382Fr,
        b:                        BN382Fr,
        end_epoch_mc_b_hash:      BN382Fr,
        prev_end_epoch_mc_b_hash: BN382Fr,
        mr_bt:                    BN382Fr,
        max_pks:                  usize,
    ) -> Self {
        ...
        let b_bool = {
            ...
            let to_skip = BN382Fr::size_in_bits() - (log_max_pks + 1);
...
impl<F: PrimeField> ConstraintSynthesizer<BN382Fr> for NaiveTresholdSignature<F> {
    fn generate_constraints<CS: ConstraintSystem<BN382Fr>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        let mut valid_signatures = BN382FrGadget::zero(cs.ns(|| "alloc valid signatures count"))?;
        ...
        let wcert_sysdata_hash_g = BN382FrPoseidonHashGadget::check_evaluation_gadget(
        ...
        let actual_aggregated_input = BN382FrPoseidonHashGadget::check_evaluation_gadget(
            cs.ns(|| "H(pks_threshold_hash, wcert_sysdata_hash)"),
            &[pks_threshold_hash_g, wcert_sysdata_hash_g]
        )?;

        let expected_aggregated_input = BN382FrGadget::alloc_input(
        ...
        let b_field = BN382FrGadget::from_bits(
...
#[allow(dead_code)]
pub fn generate_parameters(max_pks: usize) -> Result<Parameters<Bn382>, SynthesisError> {
    ...
    let c = NaiveTresholdSignature::<BN382Fr> {
    ...
    let params = generate_random_parameters::<Bn382, _, _>(c, &mut rng);
    params
}
...
mod test {
    use super::*;
    use algebra::{curves::bn_382::Bn382, BigInteger384, ProjectiveCurve};
    use primitives::{
        crh::FieldBasedHash,
        signature::{
    ...
    type SchnorrSig = FieldBasedSchnorrSignatureScheme<BN382Fr, BN382G1Projective, BN382FrPoseidonHash>;
    ...
    fn generate_test_proof(
        ...
        params:                   Parameters<Bn382>,
    ) -> Result<(Proof<Bn382>, Vec<BN382Fr>), SynthesisError> {
        ...
        let mr_bt: BN382Fr = rng.gen();
        let prev_end_epoch_mc_b_hash: BN382Fr = rng.gen();
        let end_epoch_mc_b_hash: BN382Fr = rng.gen();
        let message = h
            .update(mr_bt)
        ...
        let invalid_message: BN382Fr = rng.gen();
        ...
        //Generate b
        let t_field = BN382Fr::from_repr(BigInteger384::from(threshold as u64));
        let valid_field = BN382Fr::from_repr(BigInteger384::from(valid_sigs as u64));
        ...
        let c = NaiveTresholdSignature::<BN382Fr>::new(
